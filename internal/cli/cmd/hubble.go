...
func newCmdMichiSuperSecretHubbleEnable() *cobra.Command {
	var params = hubble.Parameters{
		Writer: os.Stdout,
	}
	...
	cmd := &cobra.Command{
		Use:   "enable",
		Short: "EXPERIMENTAL: Enable Hubble observability using Helm",
		Long:  ``,
		RunE: func(cmd *cobra.Command, args []string) error {
			params.Namespace = namespace
			ctx := context.Background()
			...
			if err := h.EnableWithHelm(ctx, k8sClient); err != nil {
				fatalf("Unable to enable Hubble: %s", err)
			}
			return nil
		},
...
func newCmdMichiSuperSecretHubbleDisable() *cobra.Command {
	var params = hubble.Parameters{
		Writer: os.Stdout,
	}
	...
	cmd := &cobra.Command{
		Use:   "disable",
		Short: "EXPERIMENTAL: Disable Hubble observability using Helm",
		Long:  ``,
		RunE: func(cmd *cobra.Command, args []string) error {
			params.Namespace = namespace
			ctx := context.Background()
			...
			if err := h.DisableWithHelm(ctx, k8sClient); err != nil {
				fatalf("Unable to disable Hubble:  %s", err)
			}
			return nil
		},
